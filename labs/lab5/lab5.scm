        ...
        (let ((word (vector-ref program index)))
          (cond
            ...
            ((equal? word 'not) (interpretator (+ index 1)
                                               ...
                                               return-stack def-stack))
            ((equal? word 'drop) (interpretator (+ index 1)
                                                (cdr stack) return-stack def-stack))
            ((equal? word 'swap) (interpretator (+ index 1)
                                                (append (list (cadr stack) (car stack)) (cddr stack))
                                                return-stack def-stack))
            ((equal? word 'dup) (interpretator (+ index 1)
                                               (cons (car stack) stack) return-stack def-stack))
            ...
            ((equal? word 'rot)
             (interpretator (+ index 1)
                            ...
                            return-stack def-stack))
            ((equal? word 'depth) (interpretator (+ index 1) (cons (length stack) stack)
                                                 return-stack def-stack))
            ((equal? word 'neg) (interpretator (+ index 1)
                                               (cons (- (car stack)) (cdr stack)) return-stack def-stack))
            ((equal? word 'define)
             (interpretator (+ (find-word 'end index program) 1) stack return-stack
                            (cons (list (vector-ref program (+ index 1)) (+ index 2)) def-stack)))
            ((contains? '(end exit) word) (interpretator (car return-stack)
                                                         stack (cdr return-stack) def-stack))
            ((equal? 'if word)
             (interpretator (if (= (car stack) 0) (+ (find-word 'endif index program)) (+ index 1))
                            (cdr stack) return-stack def-stack))
            ((equal? 'endif word) (interpretator (+ index 1) stack return-stack def-stack))
            (else (interpretator (cadr (assoc word def-stack))
                                 stack (cons (+ index 1) return-stack) def-stack)))))))
